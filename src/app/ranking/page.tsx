...
export default function RankingPage() {
  ...
  const [audit, setAudit] = useState<{ userId: string; username: string; avatar: string } | null>(null)
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const [{ data: prof }, { data: lb }, { count }] = await Promise.all([
      ...
      setProfile(prof)
      setEntries(lb || [])
      setHasFinishedMatches((count || 0) > 0)
      setLoading(false)
    }
    load()
  }, [])
    ...
    <>
      <div className="min-h-screen bg-gray-50 pb-24 md:pb-8">
        ...
        <div className="max-w-2xl mx-auto px-4 mt-4">
          ...
          {/* Dica de auditoria */}
          {hasFinishedMatches && (
            <div className="mb-3 bg-blue-50 border border-blue-100 rounded-xl px-4 py-2.5 flex items-center gap-2">
              <span className="text-blue-500">🔍</span>
              <p className="text-xs text-blue-700">
                <strong>Clique em qualquer jogador</strong> para ver os palpites dele nos jogos já encerrados.
              </p>
            </div>
          )}
          ...
          {entries.length === 0 ? (
            ...
            <div className="space-y-2">
              {entries.map((entry, i) => {
                ...
                const clickable = hasFinishedMatches

                return (
                  <button
                    key={entry.id}
                    onClick={() => clickable && setAudit({
                      userId: entry.id,
                      username: entry.username,
                      avatar: entry.avatar_emoji,
                    })}
                    className={`w-full text-left bg-white rounded-2xl border px-4 py-3 flex items-center gap-3 lb-row transition-all ${
                      isMe ? 'border-green-300 bg-green-50' : 'border-gray-100'
                    } ${clickable ? 'hover:shadow-md hover:border-blue-200 cursor-pointer active:scale-[0.99]' : 'cursor-default'}`}
                  >
                    {/* Rank */}
                    ...
                    <div className="flex-1 min-w-0">
                      <div className={`font-semibold truncate flex items-center gap-1.5 ${isMe ? 'text-green-700' : 'text-gray-800'}`}>
                        {entry.username}
                        {isMe && <span className="text-xs text-green-500 font-normal">(você)</span>}
                      </div>
                      <div className="text-xs text-gray-400 mt-0.5">
                        {entry.correct_count} acertos · {entry.total_predictions} palpites
                        {clickable && <span className="text-blue-400 ml-1">· ver detalhes →</span>}
                      </div>
                    </div>
          ...
          <div className="mt-6 bg-white rounded-2xl border border-gray-100 p-4 text-center text-sm text-gray-500">
            🔄 Ranking atualiza em tempo real conforme o admin insere resultados
          </div>
        </div>
      </div>
